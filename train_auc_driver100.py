...
class FerModel(object):
    ...
    def build_model(self):
        ...
        x = Dropout(0.3)(x)
        # x3 = self.danet(x)
        x8 = self.py_block(x3, 16, 64,"conv2d_1_att1")
    ...
    def train(self):
        ...
        for key, v in enumerate(img_list_all):
            ...
            sobel_image = np.expand_dims(sobel_image, axis=0)
            #     # 获取最后一层卷积层的输出
            last_conv_layer = self.model.get_layer('conv2d_1_att2')
            last_conv_layer_output = last_conv_layer.output
            model_with_last_conv = Model(inputs=self.model.input, outputs=last_conv_layer_output)
            feature_map = model_with_last_conv.predict(sobel_image)
            aa = visualize_feature_map(feature_map)
...
def get_heatmap(model, index, last_conv_layer_name, out_channel,sobel_image):
    ...
    # `pooled_grads` and the output feature map of last convolutional layer,
    # given a sample image
    iterate = K.function([model.input], [pooled_grads, last_conv_layer.output[0]])

    # These are the values of these two quantities, as Numpy arrays,
    # given our sample image of two elephants
    pooled_grads_value, conv_layer_output_value = iterate([sobel_image])
